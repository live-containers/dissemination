...
\jg{You might refer here to later section on CRIU.}
...
\jg{I think that there were checkpointing solutions before the appearance of virtual machines: https://en.wikipedia.org/wiki/Application_checkpointing#cite_note-1}
...
As memory pages tend to be the largest resource to dump and transfer, most pre-copy implementations iteratively transfer the memory that has changed between subsequent iterations. \jg{Well, this is true assuming that the storage is shared among the nodes, so you do not need to transfer the file system.}
...
\jg{I assume this is because of the support for established TCP connections, not because of the implementation of a distributed coordination algorithm.}
...
Other open-source tools that implement C/R are \criu, \texttt{DMTC}~\cite{dmtcp}, and \texttt{BLCR}~\cite{blcr}.
