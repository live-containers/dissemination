...
Checkpointing refers to the ability of storing the state of a computation such that it can be continued later at that same state without covering the preceeding ones.
The saved state is called a \textit{checkpoint} and the resumed process a \textit{restart} or \textit{restore}~\cite{Schulz2011}.
It provides systems with additional fault-tolerancy and fast rollback times.
...
This approach tends to be faster (lower total migration time) than pre-copy, but inccurs in service degradation as page faults become extremely costly.
